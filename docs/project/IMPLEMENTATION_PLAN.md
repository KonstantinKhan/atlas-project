# План реализации

> _Последнее обновление: 2026-03-03 (Фаза A завершена)_

Поэтапный план завершения Этапа 1 дорожной карты. Задачи сгруппированы по фазам, упорядочены по зависимостям.

**Размеры:** S = 1–2 часа | M = 3–5 часов | L = 1–2 дня

---

## Фаза A: Технический долг (подготовка) — ✅ ЗАВЕРШЕНА

> Цель: создать фундамент для безопасного добавления новых фич.

| ID | Задача | Слой | Размер | Статус |
|----|--------|------|--------|--------|
| A1 | Рефакторинг BFS-каскада в `ProjectPlan` | Backend | S | ✅ |
| A2 | Убрать `println`-дебаг из `Routing.kt` (7 шт.) | Backend | S | ✅ |
| A3 | Unit-тесты доменной модели (`ProjectPlan`, `TimelineCalendar`, `TaskSchedule`) | Backend | L | ✅ |
| A4 | Интеграционные тесты API (основные эндпоинты) | Backend | M | ✅ |

### Результаты

**A1** — Выделен приватный метод `cascadeBfs(seedTaskId, calendar)` в `ProjectPlan.kt`. Три метода (`changeTaskStartDate`, `changeTaskEndDate`, `addDependency`) используют его вместо inline BFS-кода.

**A2** — Удалены 7 строк `println("[Routing]...")` из `Routing.kt`. Не заменены на логгер.

**A3** — 25 unit-тестов:
- `TimelineCalendarTest` (12): `addWorkingDays`, `subtractWorkingDays`, `workingDaysBetween`, `currentOrNextWorkingDay` — включая выходные, праздники, пограничные случаи
- `ProjectPlanTest` (11): `changeTaskStartDate` (одиночная, каскад, цепочка), `changeTaskEndDate` (пересчёт duration, каскад), `addDependency` (FS, цикл, множественные predecessors)
- `TaskScheduleTest` (2): создание с `ProjectDate.Set` и `ProjectDate.NotSet`

**A4** — 7 интеграционных тестов Ktor-эндпоинтов с in-memory repo. `AppConfig` рефакторнут — добавлен конструктор `AppConfig(repo)` для тестов.

---

## Фаза B: Завершение 1.1 — Task Pool

> Цель: полный CRUD для задач.

| ID | Задача | Слой | Размер | Зависит от |
|----|--------|------|--------|------------|
| B1 | DELETE-эндпоинт для задачи | Backend | M | — |
| B2 | UI кнопка удаления задачи | Frontend | M | B1 |
| B3 | Диалог подтверждения удаления | Frontend | S | B2 |

### Детали

**B1** — Добавить `deleteTask(id)` в `IAtlasProjectTaskRepo` и обе реализации (Postgres, InMemory). Эндпоинт `DELETE /project-tasks/{id}`. При удалении задачи — каскадное удаление связанных `TaskSchedule` и `TaskDependency`.

**B2** — Кнопка удаления в `GanttTaskRow`. Вызов API через `projectTasksApi.deleteTask()`. Обновление React-стейта.

**B3** — Модальное окно подтверждения перед удалением. Показать, какие зависимости будут потеряны.

---

## Фаза C: Завершение 1.2 — Gantt-планирование

> Цель: полноценное интерактивное планирование через Gantt-диаграмму.

| ID | Задача | Слой | Размер | Зависит от |
|----|--------|------|--------|------------|
| C1 | Drag бара (перемещение задачи) | Frontend | L | — |
| C2 | Resize бара (изменение длительности) | Frontend | L | — |
| C3 | Backend: эндпоинт для назначения расписания | Backend | M | — |
| C4 | Назначение задачи из пула на таймлайн | Both | L | C3 |
| C5 | Планирование назад (end → start) | Both | M | — |
| C6 | Оптимистичные обновления для drag/resize | Frontend | M | C1, C2 |

### Детали

**C1** — `onMouseDown` + `onMouseMove` + `onMouseUp` на `GanttBar`. При отпускании — вычислить новую дату по позиции, отправить `ChangeStartDate`-команду. Визуальный фидбек во время drag (ghost bar).

**C2** — Resize-handle на правом краю бара. При отпускании — `ChangeEndDate`-команда. Минимальная длительность — 1 рабочий день.

**C3** — `POST /project-tasks/{id}/schedule` — принимает `{ start, duration }`, создаёт `TaskSchedule` для задачи из пула.

**C4** — Drag задачи из списка пула на область таймлайна. Определить дату по позиции drop, вызвать C3-эндпоинт.

**C5** — Новый метод в `ProjectPlan`: задаётся `end` + `duration` → вычисляется `start` через `subtractWorkingDays`. Новый UX-command + API-эндпоинт.

**C6** — Мгновенное обновление позиции бара в UI до ответа сервера. Откат при ошибке.

---

## Фаза D: Завершение 1.3 — Зависимости

> Цель: полная поддержка всех типов зависимостей с CRUD.

| ID | Задача | Слой | Размер | Зависит от |
|----|--------|------|--------|------------|
| D1 | DELETE-эндпоинт для зависимости | Backend | M | — |
| D2 | Подключить удаление зависимости к API | Frontend | S | D1 |
| D3 | UI выбора типа зависимости (FS/SS/FF/SF) | Frontend | M | — |
| D4 | Визуализация SS/FF/SF стрелок | Frontend | M | D3 |
| D5 | Пересчёт при удалении зависимости | Backend | M | D1 |

### Детали

**D1** — `DELETE /dependencies?from={id}&to={id}`. Добавить `deleteDependency` в репозиторий.

**D2** — В `handleRemoveDependency` (`GanttChart.tsx`) добавить вызов API перед обновлением стейта. Сейчас — только `setDependencies(filter(...))`.

**D3** — При создании зависимости (drag от handle) — popup с выбором типа. По умолчанию — FS.

**D4** — Разные стили стрелок для разных типов: FS (сплошная), SS (штриховая start→start), FF (штриховая end→end), SF (пунктирная).

**D5** — При удалении зависимости — пересчитать расписание затронутых задач. Задача-successor может сдвинуться назад (раньше), если ограничение снято.

---

## Фаза E: UX-улучшения для MVP

> Цель: довести пользовательский опыт до уровня MVP.

| ID | Задача | Слой | Размер | Зависит от |
|----|--------|------|--------|------------|
| E1 | Изменение статуса задачи из UI | Frontend | S | — |
| E2 | Редактирование описания задачи | Frontend | M | — |
| E3 | Обработка ошибок drag/resize | Frontend | S | C1, C2 |
| E4 | Индикатор длительности на баре | Frontend | S | — |

### Детали

**E1** — Dropdown/toggle для статуса задачи (NotStarted / InProgress / Done) в `GanttTaskRow`.

**E2** — Inline-редактирование описания задачи. PATCH `/project-tasks/{id}` уже поддерживает обновление полей.

**E3** — Toast-уведомления при ошибках drag/resize. Визуальный откат позиции бара.

**E4** — Показать количество дней на баре Gantt (текст внутри или тултип).

---

## Порядок реализации (рекомендуемый)

```
Фаза A (техдолг)
    ↓
Фаза B (Task Pool)      Фаза D (Зависимости)
    ↓                        ↓
Фаза C (Gantt-планирование)
    ↓
Фаза E (UX-улучшения)
```

Фазы B и D можно выполнять параллельно. Фаза C зависит от стабильного CRUD (B) и зависимостей (D). Фаза E — финальная полировка.

---

## Связанные документы

- [Дорожная карта](../ROAD_MAP.md)
- [Статус реализации](IMPLEMENTATION_STATUS.md)
- [Обсуждение команды](TEAM_DISCUSSION.md)
